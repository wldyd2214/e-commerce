...
## 동시성 문제
- e-커머스 서비스에서 발생할 수 있는 동시성 문제를 생각해보고 해결한 내용 기술블로그(https://jiyongpark-dev.tistory.com/113) 에 정리

---

## Query 분석 및 캐싱 전략 설계
- 조회가 오래 걸리는 쿼리에 대한 캐싱, 혹은 Redis 를 이용한 로직 이관을 통해 성능 개선할 수 있는 로직을 분석하고 이를 합리적인 이유와 함께 정리한 문서 기술블로그(https://jiyongpark-dev.tistory.com/113) 에 정리
