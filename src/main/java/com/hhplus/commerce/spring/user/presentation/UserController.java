...
import com.hhplus.commerce.spring.user.domain.service.AuthService;
...
import com.hhplus.commerce.spring.user.presentation.request.UserLoginRequest;
...
public class UserController {
    ...
    @Operation(summary = "회원 로그인 API", description = "서비스를 이용하기 위한 토큰 정보를 생성합니다.")
    @PostMapping("/login")
    public ApiResponse<UserTokenInfo> login(@RequestBody @Valid UserLoginRequest request) {
        return ApiResponse.ok(authService.createAuthToken(request.toCommand()));
    }
